...
import {Routes, Route, Outlet, useOutlet, useLocation, useNavigate} from 'react-router-dom'
      ...
      <nav className='hidden md:block w-60 sticky top-16 h-[calc(100vh_-_5rem)] overflow-y-auto bg-hei'>
        ...
        <ul className='border-r h-[calc(100vh_-_9rem_-_35px)] overflow-y-auto pb-8'>
          ...
          {navigation.map((item, index) => {
            if (item.type === 'tip') {
              return <div className='mx-2 text-sm px-4 text-gray-400 first:pt-0 pb-4 pt-8' key={item.name}>{item.name}</div>
            }
            return (
              <ActiveLink className='text-gray-700' activeName='text-blue-700' to={item.path} key={item.path} onClick={forceUpdate}>
                <li key={item.path} className='mx-2 px-4 py-3 transition hover:bg-blue-100 text-sm flex items-center hover:text-blue-700 rounded'>
